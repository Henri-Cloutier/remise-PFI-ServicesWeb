...
export const LIKE_EndPoint = function (HttpContext) {
    if(HttpContext.req.url == '/likes/AddLike' && HttpContext.req.method == "POST"){
        try {
            let likesController = new LikesController(HttpContext);
            if (HttpContext.payload)
                likesController.AddLike(HttpContext.payload);
            else
                HttpContext.response.badRequest();
            return true;
        } catch (error) {
        ...
        try {
            ...
            console.log("Likes_EndPoint Error message: \n", error.message);
            console.log("Stack: \n", error.stack);
            HttpContext.response.notFound("Like not found");
            return true;
        }
    }
